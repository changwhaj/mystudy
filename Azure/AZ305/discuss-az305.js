const PASSKEY = decoded = fromHex("415a333035");
...
function setSequence(seq, vlist) {
    ...
    setCookie("AZ305"+seq, vlist, {secure: true, 'expires': expDate});
}
...
function NewTab(question_id, discuss_id) {
    ...
    if (cntrlIsPressed || seq == "X") {
        ...
        if (passwd.toUpperCase() == PASSKEY) {
            url = "https://changwhaj.github.io/exam-assets/exam/ms/AZ305/AZ305-Q" + question_id + ".html"
        } else {
            url = "https://www.examtopics.com/discussions/microsoft/view/" + discuss_id + "-exam-az-305-topic/";
        }
...
var passwd = getCookie('passaz305');
...
if (passwd == undefined) {
    ...
    if (passwd.toUpperCase() == PASSKEY) {
        ...
        expDate.setMonth(expDate.getMonth() + 1);
        expDate = expDate.toUTCString();
        setCookie('passaz305', passwd, {secure: true, 'expires': expDate});
    }
}
passwd = getCookie('passaz305');
